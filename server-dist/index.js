...
try {
    db.exec("ALTER TABLE daily_advances ADD COLUMN driverNameOverride TEXT DEFAULT ''");
}
catch { }
try {
    db.exec("ALTER TABLE daily_advances ADD COLUMN driverNumberOverride TEXT DEFAULT ''");
}
catch { }
try {
    db.exec("ALTER TABLE daily_advances ADD COLUMN vehicleNumberOverride TEXT DEFAULT ''");
}
catch { }
try {
    db.exec("ALTER TABLE daily_advances ADD COLUMN cardNumberOverride TEXT DEFAULT ''");
}
catch { }
try {
    db.exec("ALTER TABLE daily_advances ADD COLUMN entryType TEXT DEFAULT 'LOADING'");
}
catch { }
try {
    db.exec("ALTER TABLE vehicles ADD COLUMN ton TEXT DEFAULT ''");
}
catch { }
try {
    db.exec("ALTER TABLE vehicles ADD COLUMN inchargeName TEXT DEFAULT ''");
}
catch { }
if (!db.prepare("SELECT count(*) c FROM users WHERE lower(username)='deepak'").get().c) {
    db.prepare('INSERT INTO users(username,password,name,role) VALUES(?,?,?,?)').run('deepak', 'DEEPAK12', 'Deepak (Operator)', 'user');
}
if (!db.prepare("SELECT count(*) c FROM users WHERE lower(username)='praga'").get().c) {
    db.prepare('INSERT INTO users(username,password,name,role) VALUES(?,?,?,?)').run('praga', '8667488685', 'Pragadeesh (Admin)', 'admin');
}
if (!db.prepare("SELECT count(*) c FROM users WHERE lower(username)='user'").get().c) {
    db.prepare('INSERT INTO users(username,password,name,role) VALUES(?,?,?,?)').run('user', 'user', 'Daily Advance Operator', 'user');
}
const seed = [['TN38AB4511', '4511', '123456', 'Ajit', '9876543210', '30/35', 'SIVA'], ['TN38AB0947', '0947', '234567', 'Kumar', '9876543211', '20/24', 'DEEPAK'], ['TN38AB0022', '0022', '345678', 'Ravi', '9876543212', '32/35', 'SILAMBU'], ['TN38AB0531', '0531', '456789', 'Suresh', '9876543213', '28', 'SILAMPU'], ['TN38AB1122', '1122', '567890', 'Mani', '9876543214', '30/35', 'SHIVA']];
if (!db.prepare('SELECT count(*) c FROM vehicles').get().c)
    seed.forEach(x => db.prepare('INSERT INTO vehicles(vehicleNumber,lastFourDigits,cardNumber,driverName,driverNumber,ton,inchargeName,remarks) VALUES(?,?,?,?,?,?,?,?)').run(...x, 'Sample data'));
const rows = (sql, ...v) => db.prepare(sql).all(...v), one = (sql, ...v) => db.prepare(sql).get(...v);
app.post('/api/login', (q, r) => {
    const username = String(q.body.username || '').trim().toLowerCase();
    const password = String(q.body.password || '').trim();
    const u = one('SELECT id,username,name,role FROM users WHERE lower(username)=? AND password=?', username, password);
    if (!u)
        return r.status(401).json({ message: 'Invalid username or password.' });
...
app.post('/api/users', (q, r) => {
    ...
    if (!username || !password || !name)
        return r.status(400).json({ message: 'Username, password, and name are required.' });
    try {
...
app.put('/api/users/:id', (q, r) => {
    ...
    if (!name)
        return r.status(400).json({ message: 'Name is required.' });
    if (password)
        db.prepare('UPDATE users SET password=?,name=?,role=? WHERE id=?').run(String(password).trim(), String(name).trim(), role === 'admin' ? 'admin' : 'user', q.params.id);
    else
        db.prepare('UPDATE users SET name=?,role=? WHERE id=?').run(String(name).trim(), role === 'admin' ? 'admin' : 'user', q.params.id);
...
app.post('/api/vehicles', (q, r) => { const v = vehiclePayload(q.body); if (!valid(v))
    return r.status(400).json({ message: 'Please complete all required vehicle fields; last 4 must contain exactly 4 digits.' }); try {
    const x = db.prepare('INSERT INTO vehicles(vehicleNumber,lastFourDigits,cardNumber,driverName,driverNumber,inchargeName,status,remarks,ton) VALUES(?,?,?,?,?,?,?,?,?)').run(v.vehicleNumber, v.lastFourDigits, v.cardNumber, v.driverName, v.driverNumber, v.inchargeName, v.status, v.remarks, v.ton);
    r.status(201).json(one('SELECT * FROM vehicles WHERE id=?', x.lastInsertRowid));
}
catch {
    r.status(409).json({ message: 'A vehicle with these last 4 digits already exists.' });
} });
app.put('/api/vehicles/:id', (q, r) => { const v = vehiclePayload(q.body); if (!valid(v))
    return r.status(400).json({ message: 'Invalid vehicle data' }); try {
    db.prepare('UPDATE vehicles SET vehicleNumber=?,lastFourDigits=?,cardNumber=?,driverName=?,driverNumber=?,inchargeName=?,status=?,remarks=?,ton=?,updatedAt=CURRENT_TIMESTAMP WHERE id=?').run(v.vehicleNumber, v.lastFourDigits, v.cardNumber, v.driverName, v.driverNumber, v.inchargeName, v.status, v.remarks, v.ton, q.params.id);
    r.json(one('SELECT * FROM vehicles WHERE id=?', q.params.id));
}
catch {
    r.status(409).json({ message: 'Last 4 digits must be unique.' });
} });
app.delete('/api/vehicles/:id', (q, r) => { try {
    db.prepare('DELETE FROM vehicles WHERE id=?').run(q.params.id);
    r.sendStatus(204);
}
catch {
...
app.post('/api/advances', (q, r) => { const b = q.body, date = norm(b.date), vehicleId = Number(b.vehicleId); const v = one('SELECT * FROM vehicles WHERE id=?', vehicleId); if (!date || !v || !norm(b.inchargeName) || isNaN(Number(b.totalAmount)))
    return r.status(400).json({ message: 'Complete all required fields with a valid active vehicle.' }); if (!b.allowDuplicate && one('SELECT id FROM daily_advances WHERE date=? AND vehicleId=?', date, vehicleId))
    return r.status(409).json({ message: "This vehicle has already been added for today's advance.", duplicate: true }); const driverNameInput = norm(b.driverNameOverride) || norm(b.driverName); const driverNumberInput = norm(b.driverNumberOverride) || norm(b.driverNumber); const vehicleNumberInput = norm(b.vehicleNumberOverride); const cardNumberInput = norm(b.cardNumberOverride); if (driverNameInput && (!norm(v.driverName) || v.driverName === 'Pending update')) {
    db.prepare("UPDATE vehicles SET driverName=?, updatedAt=CURRENT_TIMESTAMP WHERE id=?").run(driverNameInput, vehicleId);
} if (driverNumberInput && (!norm(v.driverNumber) || v.driverNumber === 'PENDING')) {
    db.prepare("UPDATE vehicles SET driverNumber=?, updatedAt=CURRENT_TIMESTAMP WHERE id=?").run(driverNumberInput, vehicleId);
} if (vehicleNumberInput && (norm(v.vehicleNumber).startsWith('PENDING') || !norm(v.vehicleNumber))) {
    db.prepare("UPDATE vehicles SET vehicleNumber=?, updatedAt=CURRENT_TIMESTAMP WHERE id=?").run(vehicleNumberInput, vehicleId);
} if (cardNumberInput && (norm(v.cardNumber) === 'PENDING' || !norm(v.cardNumber))) {
    db.prepare("UPDATE vehicles SET cardNumber=?, updatedAt=CURRENT_TIMESTAMP WHERE id=?").run(cardNumberInput, vehicleId);
} const tonVal = norm(b.ton) || norm(v.ton); const cardCheck = cardNumberInput || norm(v.cardNumber); const isPersonal = cardCheck.toUpperCase() === 'P/A' || cardCheck.toUpperCase() === 'PA'; const isExtra = !isPersonal && (/\bextra\b/i.test(norm(b.remarks)) || norm(b.entryType).toUpperCase() === 'EXTRA'); const entryType = isPersonal ? 'PERSONAL' : (isExtra ? 'EXTRA' : (norm(b.entryType) || 'LOADING')); const x = db.prepare('INSERT INTO daily_advances(date,vehicleId,inchargeName,ton,totalAmount,remarks,driverNameOverride,driverNumberOverride,vehicleNumberOverride,cardNumberOverride,entryType) VALUES(?,?,?,?,?,?,?,?,?,?,?)').run(date, vehicleId, norm(b.inchargeName), tonVal, Number(b.totalAmount), norm(b.remarks), driverNameInput, driverNumberInput, vehicleNumberInput, cardNumberInput, entryType); r.status(201).json(one(join + ' WHERE a.id=?', x.lastInsertRowid)); });
app.post('/api/advances/bulk', (q, r) => { const b = q.body, date = norm(b.date), incharge = norm(b.inchargeName), globalTon = norm(b.ton), entries = Array.isArray(b.entries) ? b.entries : [], group = ['LOADING', 'PERSONAL', 'EXTRA'].includes(b.entryType) ? b.entryType : 'LOADING'; if (!date || !incharge)
    return r.status(400).json({ message: 'Date and incharge name are required.' }); let added = 0, duplicates = 0, invalid = 0, pending = 0; for (const x of entries) {
    const lastFour = norm(x.lastFourDigits).padStart(4, '0');
    let v = one('SELECT * FROM vehicles WHERE lastFourDigits=?', lastFour);
    if (!v && b.allowUnmatched && /^\d{4}$/.test(lastFour)) {
        const id = db.prepare('INSERT INTO vehicles(vehicleNumber,lastFourDigits,cardNumber,driverName,driverNumber,status,remarks,ton) VALUES(?,?,?,?,?,?,?,?)').run(`PENDING-${lastFour}`, lastFour, 'PENDING', 'Pending update', 'PENDING', 'Inactive', 'Auto-created from pasted advance message — update this vehicle before future use.', norm(x.ton) || globalTon).lastInsertRowid;
        v = one('SELECT * FROM vehicles WHERE id=?', id);
    }
    if (!v || isNaN(Number(x.totalAmount))) {
        invalid++;
        continue;
    }
    if (!b.allowDuplicate && one('SELECT id FROM daily_advances WHERE date=? AND vehicleId=?', date, v.id)) {
        duplicates++;
        continue;
    }
    ...
    db.prepare('INSERT INTO daily_advances(date,vehicleId,inchargeName,ton,totalAmount,remarks,driverNameOverride,entryType) VALUES(?,?,?,?,?,?,?,?)').run(date, v.id, incharge, tonVal, Number(x.totalAmount), norm(x.remarks), norm(x.driverNameOverride), entryType);
    added++;
} r.json({ added, duplicates, invalid, pending }); });
app.put('/api/advances/:id', (q, r) => { const b = q.body; const driverNameInput = norm(b.driverNameOverride) || norm(b.driverName); const driverNumberInput = norm(b.driverNumberOverride) || norm(b.driverNumber); const vehicleNumberInput = norm(b.vehicleNumberOverride); const cardNumberInput = norm(b.cardNumberOverride); const type = ['LOADING', 'PERSONAL', 'EXTRA'].includes(norm(b.entryType).toUpperCase()) ? norm(b.entryType).toUpperCase() : 'LOADING'; db.prepare('UPDATE daily_advances SET inchargeName=?,ton=?,totalAmount=?,remarks=?,driverNameOverride=?,driverNumberOverride=?,vehicleNumberOverride=?,cardNumberOverride=?,entryType=?,updatedAt=CURRENT_TIMESTAMP WHERE id=?').run(norm(b.inchargeName), norm(b.ton), Number(b.totalAmount), norm(b.remarks), driverNameInput, driverNumberInput, vehicleNumberInput, cardNumberInput, type, q.params.id); r.json(one(join + ' WHERE a.id=?', q.params.id)); });
app.delete('/api/advances/:id', (q, r) => { db.prepare('DELETE FROM daily_advances WHERE id=?').run(q.params.id); r.sendStatus(204); });
app.get('/api/dashboard', (q, r) => {
    const date = norm(q.query.date);
    const s = one('SELECT count(*) vehicles, COALESCE(sum(totalAmount),0) amount FROM daily_advances WHERE date=?', date);
    const tonCount = one("SELECT GROUP_CONCAT(ton, ', ') t FROM (SELECT ton FROM daily_advances WHERE date=? AND ton!='' GROUP BY ton)", date);
    const activeVehicles = one("SELECT count(*) c FROM vehicles WHERE status='Active'").c;
...
function exportThreeSetsExcel(data, dateText) {
    ...
    for (const type of ['LOADING', 'PERSONAL', 'EXTRA']) {
        const group = data.filter((x) => (x.entryType || 'LOADING') === type);
        const sectionTotal = group.reduce((n, x) => n + Number(x.totalAmount || 0), 0);
...
app.post('/api/import/vehicles', upload.single('file'), (q, r) => { try {
    ...
    for (const row of incoming) {
        const getVal = (...keys) => { for (const k of Object.keys(row)) {
            const cleanK = k.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
            if (keys.some(key => cleanK === key.toLowerCase().replace(/[^a-z0-9]/g, '')))
                return String(row[k]).trim();
        } return ''; };
        const vehNo = getVal('vehicleno', 'fullvehiclenumber', 'vehiclenumber', 'vehicle');
        const last4 = getVal('last4', 'lastfourdigits', 'last4digits');
        const cardNo = getVal('cardno', 'cardnumber', 'card');
        const driverName = getVal('drivername', 'driver');
        const driverNumber = getVal('driverno', 'drivernumber', 'drivermobile', 'mobile');
        const inchargeName = getVal('inchargename', 'incharge', 'incharge name');
        const ton = getVal('ton', 'capacity');
        ...
        const existing = one('SELECT id FROM vehicles WHERE lastFourDigits=?', v.lastFourDigits);
        if (existing) {
            db.prepare('UPDATE vehicles SET vehicleNumber=?,cardNumber=?,driverName=?,driverNumber=?,inchargeName=?,status=?,remarks=?,ton=?,updatedAt=CURRENT_TIMESTAMP WHERE id=?').run(v.vehicleNumber, v.cardNumber, v.driverName, v.driverNumber, v.inchargeName, v.status, v.remarks, v.ton, existing.id);
            updated++;
        }
        else {
            db.prepare('INSERT INTO vehicles(vehicleNumber,lastFourDigits,cardNumber,driverName,driverNumber,inchargeName,status,remarks,ton) VALUES(?,?,?,?,?,?,?,?,?)').run(v.vehicleNumber, v.lastFourDigits, v.cardNumber, v.driverName, v.driverNumber, v.inchargeName, v.status, v.remarks, v.ton);
            imported++;
        }
    }
